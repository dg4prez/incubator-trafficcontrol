...
sub gen_crconfig_json {
    ...
    my $rs_caches = $self->db->resultset('Server')->search(
        {
            ...
            columns  => [ 'host_name', 'domain_name', 'tcp_port', 'https_port',   'interface_name', 'ip_address', 'ip6_address', 'id', 'xmpp_id', 'profile.ccr_routing_disabled' ]
        }
    );
    ...
    while ( my $row = $rs_caches->next ) {
        ...
        if ( $row->type->name eq "RASCAL" ) {
            ...
            $data_obj->{'contentServers'}->{ $row->host_name }->{'routingDisabled'} = $row->profile->ccr_routing_disabled;
        }
    }
    my $regexps;
