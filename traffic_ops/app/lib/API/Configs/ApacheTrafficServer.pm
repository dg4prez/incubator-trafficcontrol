...
sub cdn_ds_data {
	...
	while ( my $row = $rs->next ) {
		...
		if ( $re_type eq 'HOST_REGEXP' && $ds_type ne 'ANY_MAP' ) {
			...
			if ( $host_regex =~ /\.\*$/ ) {
				...
				if ( $protocol == HTTP ) {
					...
					my $map_from2 = "https://" . $hname . $regex . $ds_domain . "/";
					$dsinfo->{dslist}->[$j]->{"remap_line2"}->{$map_from2} = $map_to;
				}
			}
			else {
				my $map_from = "http://" . $host_regex . "/";
				if ( $protocol == HTTP ) {
					$dsinfo->{dslist}->[$j]->{"remap_line"}->{$map_from} = $map_to;
				}
				elsif ( $protocol == HTTPS || $protocol == HTTP_TO_HTTPS ) {
					$map_from = "https://" . $host_regex . "/";
					$dsinfo->{dslist}->[$j]->{"remap_line"}->{$map_from} = $map_to;
				}
				elsif ( $protocol == HTTP_AND_HTTPS ) {
		...
		if ( defined($edge_header_rewrite) ) {
			my $filename = "hdr_rw_" . $ds_xml_id . ".config";
			$dsinfo->{dslist}->[$j]->{"hdr_rw_file"} = $filename;
		}
		if ( defined($mid_header_rewrite) ) {
			my $filename = "hdr_rw_mid_" . $ds_xml_id . ".config";
			$dsinfo->{dslist}->[$j]->{"mid_hdr_rw_file"} = $filename;
		}
		if ( defined($cacheurl) ) {
			my $filename = "cacheurl_" . $ds_xml_id . ".config";
			$dsinfo->{dslist}->[$j]->{"cacheurl_file"} = $filename;
		}
...
sub ds_data {
	...
	while ( my $dsinfo = $rs_dsinfo->next ) {
		my $org_server_fqdn             = $dsinfo->org_server_fqdn;
		my $dscp                        = $dsinfo->dscp;
		...
		if ( $re_type eq 'HOST_REGEXP' && $ds_type ne 'ANY_MAP' ) {
			my $host_regex = $dsinfo->pattern;
			my $map_to  = $org_server_fqdn . "/";
			if ( $host_regex =~ /\.\*$/ ) {
				my $regex = $host_regex;
				$regex =~ s/\\//g;
				$regex =~ s/\.\*//g;
		...
		$response_obj->{dslist}->[$j]->{"org"}                         = $org_server_fqdn;
		...
		if ( defined($edge_header_rewrite) ) {
			my $filename = "hdr_rw_" . $ds_xml_id . ".config";
			$response_obj->{dslist}->[$j]->{"hdr_rw_file"} = $filename;
		}
		if ( defined($mid_header_rewrite) ) {
			my $filename = "hdr_rw_mid_" . $ds_xml_id . ".config";
			$response_obj->{dslist}->[$j]->{"mid_hdr_rw_file"} = $filename;
		}
		if ( defined($cacheurl) ) {
			my $filename = "cacheurl_" . $ds_xml_id . ".config";
			$response_obj->{dslist}->[$j]->{"cacheurl_file"} = $filename;
		}
