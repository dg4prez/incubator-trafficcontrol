...
sub get_metadata {
	...
	if ($server_obj) {
		...
		$data_obj->{'info'}->{'profileName'}	 = $server_obj->profile->name;
		$data_obj->{'info'}->{'profileId'}		 = $server_obj->profile->id;
		$data_obj->{'info'}->{'cdnName'}		 = $server_obj->cdn->name;
		$data_obj->{'info'}->{'cdnId'}			 = $server_obj->cdn->id;
	...
	$data_obj->{'dsCount'} = @ds_list;
	$data_obj->{'dsList'} = \@ds_list;
